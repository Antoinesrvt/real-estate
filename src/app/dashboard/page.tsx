...
import { useQueryStates, parseAsString, parseAsFloat, parseAsJson, parseAsArrayOf, createParser } from 'nuqs';
...
// Create a custom parser for RentalUnit array
const parseAsRentalUnits = createParser({
...
const RealEstateCalculator: React.FC = () => {
  ...
  const [saleAnalysis, setSaleAnalysis] = useState<any>(null);
  ...
  const handleBaseInvestmentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setStates({ baseInvestment: e.target.value });
  };

  const handleRenovationCostChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setStates({ renovationCost: e.target.value });
  };

  const handleMarketAppreciationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setStates({ marketAppreciation: e.target.value });
  };

  const handleCityAverageReturnChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const analysis = Array.from({ length: 10 }, (_, year) => {
    const yearNumber = year + 1;
    const appreciatedValue = totalInvestment * Math.pow(1 + appreciationRate, yearNumber);
    const cumulativeRentalIncome = yearlyReturn * yearNumber;
    const totalReturn = appreciatedValue + cumulativeRentalIncome - totalInvestment;
    const annualizedReturn = (Math.pow((appreciatedValue + cumulativeRentalIncome) / totalInvestment, 1/yearNumber) - 1) * 100;
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600">
                    Based on your current returns, you'll break even on your
                ...
                <Card className="bg-gray-50">
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2 text-sm text-gray-600">
                      ...
                      {saleAnalysis.find(
                        (year: any) => year.annualizedReturn > 15
                      ) ? (
                        <p>
                          🎯{" "}
                          <span className="font-medium">
